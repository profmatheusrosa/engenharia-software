# Lista de Imagens a Gerar

- [x] **Módulo:** 1
  - **Placeholder:** `[IMAGEM_01_CICLO_DE_VIDA_SOFTWARE]`
  - **Descrição:** Um diagrama circular ilustrando o ciclo de vida do desenvolvimento de software moderno, com as fases: Planejamento, Análise, Design, Implementação, Testes e Manutenção, conectadas em um fluxo contínuo (DevOps/Agile).
  - **Prompt:** A circular diagram showing the software development life cycle (SDLC) with phases: 'Planejamento', 'Análise', 'Design', 'Implementação', 'Testes', 'Manutenção'. Modern, flat design, professional color palette (blues and purples), white background. Text in Portuguese.

- [x] **Módulo:** 1
  - **Placeholder:** `[IMAGEM_02_CARACTERISTICAS_QUALIDADE_ISO25010]`
  - **Descrição:** Um mapa mental ou diagrama de árvore mostrando as principais características da qualidade de software segundo a ISO 25010. O nó central é "Qualidade do Produto" e os ramos são: Adequação Funcional, Eficiência de Desempenho, Compatibilidade, Usabilidade, Confiabilidade, Segurança, Manutenibilidade e Portabilidade.
  - **Prompt:** A tree diagram or mind map of ISO 25010 software quality characteristics. Central node 'Qualidade de Software'. Branches: 'Funcionalidade', 'Performance', 'Compatibilidade', 'Usabilidade', 'Confiabilidade', 'Segurança', 'Manutenibilidade', 'Portabilidade'. Clean, corporate style, high resolution. Text in Portuguese.

- [x] **Módulo:** 1
  - **Placeholder:** `[IMAGEM_03_QUADRO_KANBAN]`
  - **Descrição:** Uma ilustração de um quadro Kanban clássico com três colunas: "A Fazer", "Em Progresso" e "Feito". Post-its coloridos nas colunas. Foco visual na simplicidade e fluxo.
  - **Prompt:** A visualization of a Kanban board with three columns titled 'A Fazer', 'Em Progresso', 'Feito'. Colorful sticky notes in each column. Modern UI design style, clean lines, soft shadows. Text in Portuguese.

- [x] **Módulo:** 2
  - **Placeholder:** `[IMAGEM_04_TECNICA_INVEST]`
  - **Descrição:** Um infográfico explicando a sigla INVEST para User Stories. Cada letra (I, N, V, E, S, T) deve ter um ícone representativo e a palavra por extenso (Independente, Negociável, Valiosa, Estimável, Pequena - Small, Testável). Estilo limpo e colorido.
  - **Prompt:** Infographic of the INVEST acronym for User Stories. Letters I, N, V, E, S, T with icons. Labels: 'Independente', 'Negociável', 'Valiosa', 'Estimável', 'Pequena', 'Testável'. Modern, flat design, colorful icons on white background. Text in Portuguese.

- [x] **Módulo:** 2
  - **Placeholder:** `[IMAGEM_05_USER_STORY_MAPPING]`
  - **Descrição:** Diagrama ilustrando um User Story Mapping. No topo, uma linha do tempo com a jornada do usuário (passos macro). Abaixo, cartões (post-its) organizados em colunas representando os detalhes de cada passo, divididos horizontalmente por "Releases".
  - **Prompt:** User Story Mapping diagram. Horizontal timeline at the top representing user journey. Columns of sticky notes below each step. Horizontal lines dividing 'Release 1', 'Release 2'. Post-it style visuals. Clean, professional look. Text in Portuguese.

- [x] **Módulo:** 2
  - **Placeholder:** `[IMAGEM_06_MODELO_DOMINIO_ECOMMERCE]`
  - **Descrição:** Um diagrama de modelo de domínio simples para um e-commerce. Entidades: Cliente, Pedido, Produto, Carrinho. Relacionamentos mostrados com linhas simples e verbos (Cliente "faz" Pedido). Sem detalhes técnicos de banco de dados, focado em conceitos.
  - **Prompt:** A conceptual Domain Model diagram for an E-commerce. Entities: 'Cliente', 'Pedido', 'Produto', 'Carrinho'. Simple lines connecting them with relationship labels. Conceptual view, no database keys or types. Clean UML-like boxes but simpler. Text in Portuguese.

- [x] **Módulo:** 3
  - **Placeholder:** `[IMAGEM_07_UML_ESSENCIAL_COLLAGE]`
  - **Descrição:** Uma colagem mostrando exemplos simplificados dos três diagramas UML mais úteis: Caso de Uso (bonequinho ator + elipse), Classes (retângulo com atributos/métodos) e Sequência (linhas de vida verticais e setas horizontais).
  - **Prompt:** A collage of three simple UML diagrams side-by-side: Use Case (stick figure and ellipse), Class (rectangle with text), Sequence (vertical lifelines and arrows). Clean, educational style, white background. Labels: 'Caso de Uso', 'Classes', 'Sequência'. Text in Portuguese.

- [x] **Módulo:** 3
  - **Placeholder:** `[IMAGEM_08_MODELO_C4_ZOOM]`
  - **Descrição:** Uma ilustração conceitual do Modelo C4 mostrando os 4 níveis de zoom como se fossem lentes de aumento. Nível 1 (Contexto) -> Nível 2 (Containers) -> Nível 3 (Componentes) -> Nível 4 (Código).
  - **Prompt:** Conceptual illustration of the C4 Model showing 4 levels of zoom like magnifying glasses. Level 1: Context, Level 2: Containers, Level 3: Components, Level 4: Code. Abstract, modern tech style. Text in Portuguese.

- [x] **Módulo:** 4
  - **Placeholder:** `[IMAGEM_09_CLEAN_ARCHITECTURE_ONION]`
  - **Descrição:** Diagrama circular de "cebola" para Clean Architecture. Círculo Central: Entidades. Círculo do Meio: Casos de Uso. Círculo Externo: Controllers/Gateways. Círculo Mais Externo: UI/DB/Devices. Setas apontando para dentro.
  - **Prompt:** Concentric circles diagram representing Clean Architecture (The Onion Architecture). Inner circle: 'Entities'. Middle: 'Use Cases'. Outer: 'Interface Adapters'. Outermost: 'Frameworks & Drivers'. Arrows pointing inwards. Clean, technical style. Text in Portuguese.

- [x] **Módulo:** 4
  - **Placeholder:** `[IMAGEM_10_ARQUITETURA_HEXAGONAL]`
  - **Descrição:** Diagrama de um hexágono representando a Aplicação (Core). No lado esquerdo (Input), setas entrando (API, UI, Testes). No lado direito (Output), setas saindo para Adaptadores (Banco de Dados, Email, Filas).
  - **Prompt:** Hexagonal Architecture diagram (Ports and Adapters). Central hexagon labelled 'Core Application'. Left side input arrows: 'API', 'GUI', 'Test'. Right side output arrows: 'Database', 'Email', 'Queue'. Professional technical diagram style. Text in Portuguese.

- [x] **Módulo:** 5
  - **Placeholder:** `[IMAGEM_11_PRINCIPIOS_SOLID]`
  - **Descrição:** Uma composição visual com 5 quadros, cada um representando um princípio SOLID com uma metáfora visual simples (Ex: SRP = Canivete vs Faca Especializada; LSP = Pato de Borracha). Estilo clean e iconográfico.
  - **Prompt:** A composition of 5 simple icons representing SOLID principles. S: Single Responsibility, O: Open/Closed, L: Liskov Substitution, I: Interface Segregation, D: Dependency Inversion. Minimalist, modern iconography. Text in Portuguese where applicable.

- [ ] **Módulo:** 5
  - **Placeholder:** `[IMAGEM_12_DESIGN_PATTERNS_ICONS]`
  - **Descrição:** Ícones representando padrões de design comuns: Engrenagem (Factory), Tijolos (Builder), Adaptador de tomada (Adapter), Fachada de prédio (Facade), Olho (Observer).
  - **Prompt:** A set of 5 minimal icons representing Design Patterns: Factory (Gear), Builder (Bricks), Adapter (Plug adapter), Facade (Building front), Observer (Eye). Clean lines, unified style.

- [ ] **Módulo:** 6
  - **Placeholder:** `[IMAGEM_13_PIRAMIDE_DE_TESTES]`
  - **Descrição:** Pirâmide de Testes clássica. Base larga: "Testes Unitários (Rápido/Barato)". Meio: "Testes de Integração". Topo estreito: "Testes E2E (Lento/Caro)". Nuvem ou Olho no topo representando Testes Manuais.
  - **Prompt:** The Test Pyramid diagram. Wide base labeled 'Unit Tests'. Middle layer 'Integration Tests'. Narrow top 'E2E Tests'. Cloud at the very top 'Manual Tests'. Arrow on side showing 'Speed/Cost'. Clean, educational infographic. Text in Portuguese.

- [ ] **Módulo:** 6
  - **Placeholder:** `[IMAGEM_14_FLUXO_CODE_REVIEW]`
  - **Descrição:** Diagrama de fluxo simples: Autor abre Pull Request -> CI roda testes (Automático) -> Revisor comenta -> Autor corrige -> Revisor aprova -> Merge.
  - **Prompt:** A flowchart showing the Code Review process. Steps: 'Open PR', 'CI Tests', 'Review', 'Changes', 'Approved', 'Merge'. Linear flow with a loop for changes. Modern flowchart style. Text in Portuguese.

- [ ] **Módulo:** 7
  - **Placeholder:** `[IMAGEM_15_GIT_FLOW_VS_TRUNK]`
  - **Descrição:** Comparativo visual Lado a Lado. Esquerda (Git Flow): Muitas branches paralelas (develop, release, feature). Direita (Trunk-Based): Uma linha Mestra grossa com pequenas branches saindo e voltando rápido.
  - **Prompt:** Split screen comparison diagram. Left: Git Flow (complex branching, multiple parallel lines). Right: Trunk-Based Development (single thick line with short offshoots). Labeled 'Git Flow' and 'Trunk-Based'. Clean, technical style. Text in Portuguese.

- [ ] **Módulo:** 7
  - **Placeholder:** `[IMAGEM_16_PIPELINE_CICD]`
  - **Descrição:** Visualização horizontal de uma Pipeline. Ícones conectados por setas: Código (Git) -> Build (Engrenagem) -> Teste (Checklist) -> Deploy (Foguete) -> Monitoramento (Gráfico).
  - **Prompt:** A horizontal CI/CD Pipeline visualization. Icons connected by arrows: Code (Git logo), Build (Gear), Test (Checklist), Deploy (Rocket), Monitor (Graph). Modern, sleek interface style. Text in Portuguese.
